...
async function loadTeamSeasons(seasonCode) {
...
async function buildTeamSeasonDetail(teamSeasonId, calendarYear) {
  const teamSeason = await loadTeamSeason(teamSeasonId);
  const [roster, games] = await Promise.all([
    loadRoster(teamSeasonId),
    loadGames(teamSeasonId)
  ]);
...
async function handleApi(req, res, url) {
  try {
    ...
    if (url.pathname === "/api/seasons") {
      sendJson(res, 200, await loadSeasons());
      return;
    }

    if (url.pathname === "/api/overview") {
      const seasonCode = url.searchParams.get("seasonCode");
      if (!seasonCode) {
        sendJson(res, 400, { error: "Chybí seasonCode." });
        return;
      }
      sendJson(res, 200, await buildOverview(seasonCode));
      return;
    }

    if (url.pathname === "/api/team-season") {
      const teamSeasonId = url.searchParams.get("id");
      const calendarYear = url.searchParams.get("calendarYear");
      if (!teamSeasonId) {
        sendJson(res, 400, { error: "Chybí id team_season." });
        return;
      }
      sendJson(res, 200, await buildTeamSeasonDetail(teamSeasonId, calendarYear));
      return;
    }
    ...
    if (url.pathname === "/api/game") {
      ...
      if (!gameId) {
        sendJson(res, 400, { error: "Chybí id utkání." });
        return;
      }
      sendJson(res, 200, await buildGameDetail(gameId));
      return;
    }

    sendJson(res, 404, { error: "API route not found." });
  } catch (error) {
    sendJson(res, 500, { error: error.message || "Neočekávaná chyba." });
  }
}
...
server.listen(PORT, () => {
  console.log(`Presentation UI listening on http://localhost:${PORT}`);
});
